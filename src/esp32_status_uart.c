...
#define MOD_CTRL_MASK (BIT(0) | BIT(4))
#define MOD_SHIFT_MASK (BIT(1) | BIT(5))
#define MOD_ALT_MASK (BIT(2) | BIT(6))
#define MOD_GUI_MASK (BIT(3) | BIT(7))
...
static void send_status_line(void) {
    ...
    snprintf(line, sizeof(line),
             "layer=%s usb=%d bt=%d bt_slot=%d ctrl=%d alt=%d win=%d shift=%d wpm=%d "
             "peripherals=%d batt=",
             layer_value, usb_connected ? 1 : 0, bt_connected ? 1 : 0, bt_slot,
             (current_modifiers & MOD_CTRL_MASK) ? 1 : 0,
             (current_modifiers & MOD_ALT_MASK) ? 1 : 0,
             (current_modifiers & MOD_GUI_MASK) ? 1 : 0,
             (current_modifiers & MOD_SHIFT_MASK) ? 1 : 0, current_wpm, count);
...
static int esp32_status_listener(const zmk_event_t *eh) {
    const struct zmk_modifiers_state_changed *modifiers_event =
        as_zmk_modifiers_state_changed(eh);
    if (modifiers_event != NULL) {
        if (modifiers_event->state) {
            current_modifiers |= modifiers_event->modifiers;
        } else {
            current_modifiers &= ~modifiers_event->modifiers;
        }
    }
